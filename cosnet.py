...
class COSNet(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        if pretrained is not None:
            ...
            # Handle cases where checkpoint might just be the dictionary itself
            loaded_dict = checkpoint.get("state_dict", checkpoint) 
            
            new_state_dict = {}
            for key, val in loaded_dict.items():
                # Automatically map keys to the 'backbone' submodule if needed
                ...
                if val.shape != cur_state_dict[new_key].shape:
                    # Ensure we only apply 2D interpolation to 4D tensors (e.g., conv weights)
                    if val.dim() == 4: 
                        val = F.interpolate(val, size=cur_state_dict[new_key].shape[2:], mode='bilinear', align_corners=True)
                    else:
                        print(f"Skipping {new_key} due to unresolvable shape mismatch: {val.shape} vs {cur_state_dict[new_key].shape}")
                        continue
                        
                new_state_dict[new_key] = val

            # Load the adjusted dictionary
